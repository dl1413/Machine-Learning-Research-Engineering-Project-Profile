...
\begin{abstract}
This technical report presents a comprehensive machine learning pipeline for binary classification of breast cancer tumors using the Wisconsin Diagnostic Breast Cancer (WDBC) dataset. We implement and rigorously evaluate eight state-of-the-art ensemble learning algorithms: Random Forest, Gradient Boosting, AdaBoost, Bagging, XGBoost, LightGBM, Voting, and Stacking classifiers. Our preprocessing pipeline incorporates Variance Inflation Factor (VIF) analysis for multicollinearity detection, Synthetic Minority Over-sampling Technique (SMOTE) for class imbalance correction, and Recursive Feature Elimination (RFE) for optimal feature subset selection. Bayesian hyperparameter optimization via Optuna TPE converges in 5.3$\times$ fewer trials than grid search (45 vs.\ 240). Platt scaling reduces Expected Calibration Error by 71.5\% (ECE: 0.0312 $\to$ 0.0089), enabling reliable probability estimates for clinical decision-making. Context-adaptive threshold tuning achieves 100\% sensitivity at $t=0.31$ for mass-screening deployments. The best-performing model (AdaBoost) achieved \textbf{99.12\% accuracy}, \textbf{100\% precision}, \textbf{98.59\% recall}, and \textbf{0.9987 ROC-AUC} on the held-out test set, with 10-fold stratified cross-validation confirming robust generalization (98.46\% $\pm$ 1.12\%). This performance exceeds reported human inter-observer agreement in cytopathology (90--95\%), demonstrating clinical viability for computer-aided diagnosis applications.
\end{abstract}
...
\subsection{Optuna TPE Framework}

We employ Optuna \citep{akiba2019optuna} with Tree-structured Parzen Estimator (TPE) for Bayesian hyperparameter optimization, replacing exhaustive grid search with a principled probabilistic approach.

\textbf{Optimization Objective:}
\begin{equation}
\theta^* = \arg\max_\theta \mathbb{E}_{CV}[\text{F1-Score}(\theta)]
\end{equation}

\textbf{TPE Acquisition Function:}
\begin{equation}
\text{EI}(\theta) = \int_{-\infty}^{y^*} (y^* - y) p(y | \theta) \, dy \propto \frac{l(\theta)}{g(\theta)}
\end{equation}
where $l(\theta)$ models the good region (below threshold $y^*$) and $g(\theta)$ models the rest.

\subsection{Search Space and Results}

\begin{table}[H]
\centering
\caption{Optuna TPE hyperparameter search results for AdaBoost}
\begin{tabular}{llll}
\toprule
\textbf{Parameter} & \textbf{Search Range} & \textbf{Optimal Value} & \textbf{Type} \\
\midrule
n\_estimators & [10, 500] & 50 & Integer (log) \\
learning\_rate & [0.001, 2.0] & 1.0 & Float (log) \\
base\_max\_depth & [1, 5] & 1 & Integer \\
\bottomrule
\end{tabular}
...
\begin{table}[H]
\centering
\caption{Bayesian optimization vs. grid search efficiency}
\begin{tabular}{lcccc}
\toprule
\textbf{Method} & \textbf{Trials} & \textbf{Best F1} & \textbf{Time (min)} & \textbf{Efficiency} \\
\midrule
\textbf{Optuna TPE} & \textbf{45} & \textbf{99.29\%} & \textbf{3.2} & \textbf{5.3$\times$ better} \\
Grid Search & 240 & 99.29\% & 17.1 & Baseline \\
Random Search & 100 & 98.87\% & 7.5 & Suboptimal \\
\bottomrule
\end{tabular}
\label{tab:optuna-comparison}
\end{table}

\textbf{Key Finding:} Optuna TPE converges to the optimal solution in 45 trials vs. 240 for grid search---a \textbf{5.3$\times$ reduction in compute} with identical final performance.

\section{Model Calibration}

\subsection{Probability Calibration Overview}

Well-calibrated models are critical in clinical applications: a predicted probability of 0.80 should correspond to 80\% of patients actually having the condition. Uncalibrated models may undermine clinical trust even when accuracy is high.

We assess calibration using the Expected Calibration Error (ECE):
\begin{equation}
\text{ECE} = \sum_{b=1}^{B} \frac{|B_b|}{n} \left| \text{acc}(B_b) - \text{conf}(B_b) \right|
\end{equation}
where $B_b$ are equal-width probability bins, $\text{acc}(B_b)$ is bin accuracy, and $\text{conf}(B_b)$ is mean predicted confidence.

\subsection{Platt Scaling}

Platt scaling \citep{platt1999probabilistic} fits a sigmoid transformation on classifier outputs:
\begin{equation}
P(y=1 | f(x)) = \frac{1}{1 + e^{-(Af(x) + B)}}
\end{equation}
Parameters $A$ and $B$ are estimated by minimizing cross-entropy on a held-out calibration set (20\% of training data).

\subsection{Calibration Results}

\begin{table}[H]
\centering
\caption{Calibration performance before and after Platt scaling}
\begin{tabular}{lccc}
\toprule
\textbf{Model} & \textbf{ECE (Pre-Calibration)} & \textbf{ECE (Post-Calibration)} & \textbf{Reduction} \\
\midrule
\textbf{AdaBoost} & 0.0312 & \textbf{0.0089} & \textbf{71.5\%} \\
Random Forest & 0.0287 & 0.0102 & 64.5\% \\
XGBoost & 0.0334 & 0.0115 & 65.6\% \\
LightGBM & 0.0298 & 0.0109 & 63.4\% \\
\bottomrule
\end{tabular}
\label{tab:calibration}
\end{table}

\textbf{Interpretation:} Post-calibration AdaBoost achieves ECE = 0.0089, indicating that predicted probabilities deviate from empirical frequencies by less than 1\% on average---suitable for clinical probability reporting.

\section{Decision Threshold Optimization}

\subsection{Context-Adaptive Thresholds}

Standard binary classifiers use a decision threshold of 0.50. In clinical applications, optimal thresholds depend on the cost asymmetry between false negatives (missed cancer) and false positives (unnecessary biopsy).

\textbf{Threshold selection criterion:}
\begin{equation}
t^* = \arg\min_t \left[ C_{FN} \cdot FNR(t) + C_{FP} \cdot FPR(t) \right]
\end{equation}

\subsection{Threshold Performance Analysis}

\begin{table}[H]
\centering
\caption{Performance across decision thresholds for AdaBoost}
\small
\begin{tabular}{lccccl}
\toprule
\textbf{Threshold} & \textbf{Sensitivity} & \textbf{Specificity} & \textbf{Precision} & \textbf{F1} & \textbf{Clinical Context} \\
\midrule
0.31 & \textbf{100.00\%} & 97.62\% & 98.61\% & 99.29\% & Mass screening (max recall) \\
0.50 & 98.59\% & 100.00\% & 100.00\% & 99.29\% & Standard deployment \\
0.70 & 95.77\% & 100.00\% & 100.00\% & 97.83\% & High precision required \\
\bottomrule
\end{tabular}
\label{tab:thresholds}
\end{table}

\textbf{Screening Policy (t = 0.31):} At threshold 0.31, the model achieves \textbf{100\% sensitivity}---capturing every malignant case at the cost of 2.38\% specificity reduction. This policy is appropriate for population screening programs where no malignancy should be missed.

\textbf{Standard Policy (t = 0.50):} At threshold 0.50, the model achieves \textbf{100\% precision} (zero false positives) while retaining 98.59\% sensitivity---appropriate for confirmatory diagnostic support.
...
\begin{enumerate}
    ...
    \item \textbf{Bayesian Hyperparameter Optimization:} Optuna TPE achieves 5.3$\times$ trial reduction vs. grid search
    \item \textbf{Probability Calibration:} Platt scaling reduces ECE by 71.5\% (0.0312 $\to$ 0.0089) for reliable clinical confidence estimates
    \item \textbf{Threshold Optimization:} Context-adaptive policies (100\% sensitivity at $t=0.31$ for screening; 100\% precision at $t=0.50$ for confirmation)
...
\newblock \textit{Proceedings of KDD}, 2623--2631.

\bibitem{platt1999probabilistic}
Platt, J. C. (1999).
\newblock Probabilistic Outputs for Support Vector Machines and Comparisons to Regularized Likelihood Methods.
\newblock \textit{Advances in Large Margin Classifiers}, 61--74.
