...
def verify(pred, session):
    ...
    for task in CLASSES:
        ...
        if old != new:
            ...
            # Version 0.1.1: correction requires strong evidence and a weak initial proposal.
            # These raw-score gates are design constants, not tuned on held-out outcomes.
            if evidence[task][new] >= .85 and pred[task][old] < .8:
                result[task]=evidence[task]
                changes.append(task)
            else:
                # Flag unresolved disagreement by shrinking confidence, retaining the initial label.
                result[task]={c:.5*pred[task][c]+.5/len(CLASSES[task]) for c in CLASSES[task]}
        else:
            result[task]={c:(pred[task][c]+evidence[task][c])/2 for c in CLASSES[task]}
    return validate_prediction(result),{"changed_tasks":changes,"disagreed_tasks":disagreements,
                                        "before":before,"evidence_prediction":evidence}
